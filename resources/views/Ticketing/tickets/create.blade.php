                ...
                <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                        ...
                        <p class="mt-1 max-w-2xl text-sm leading-6 text-slate-500">
                            @if($isCrew)
                                Silakan isi laporan kendala outlet dengan lengkap agar tim terkait dapat menindaklanjuti lebih
                                cepat.
                            @else
                    ...
                    @if($isCrew)
                        <span
                            class="inline-flex w-fit items-center rounded-full border border-blue-100 bg-blue-50 px-4 py-2 text-xs font-bold text-blue-700">
                            Akses Crew
                        </span>
                    @endif
                </div>
                ...
                <section class="border-b border-slate-100 p-6 lg:border-b-0 lg:border-r">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            @if($isCrew)
                                {{-- TAMPILAN KHUSUS CREW: Input text biasa (Readonly), tidak ada dropdown --}}
                                <input type="text"
                                    class="ticket-input bg-slate-100 text-slate-700 font-semibold cursor-not-allowed"
                                    value="{{ $userOutlet->nama_outlet ?? 'Data outlet belum disetting' }}{{ !empty($userOutlet->kota) ? ' - ' . $userOutlet->kota : '' }}"
                                    readonly>

                                {{-- Hidden input wajib ada agar form tetap mengirim ID outlet saat disubmit --}}
                                <input type="hidden" name="outlet_id" id="outlet_id" value="{{ $userOutlet->id ?? '' }}"
                                    data-kota="{{ $userOutlet->kota ?? '' }}" data-provinsi="{{ $userOutlet->provinsi ?? '' }}">
                            @else
                                {{-- TAMPILAN KHUSUS BACKOFFICE: Dropdown Select2 biasa --}}
                                <select id="outlet_id" name="outlet_id" class="select2 ticket-input" required>
                                    <option value="">Pilih Outlet</option>
                                    @foreach($uniqueOutlets as $outlet)
                                        <option value="{{ $outlet->id }}" data-kota="{{ $outlet->kota ?? '' }}"
                                            data-provinsi="{{ $outlet->provinsi ?? '' }}" @selected(old('outlet_id') == $outlet->id)>
                        ...
                        <div>
                            ...
                            <select id="division_select" class="ticket-input bg-slate-50 text-slate-600" disabled>
                ...
                <section class="p-6">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <select id="item_select" name="item" class="ticket-input">
                                ...
                                @foreach($itemsWithOwner ?? [] as $it)
                                    <option value="{{ $it->item }}" data-owner="{{ $it->owner ?? '' }}"
                        ...
                        <div>
                            ...
                            <input id="custom_item" type="text" name="custom_item" value="{{ old('custom_item') }}"
                                placeholder="Isi jika item tidak tersedia" class="ticket-input">
                        </div>
                        ...
                        <div>
                            ...
                            <input id="leader_name" type="text" name="leader_name"
                                value="{{ old('leader_name', $isCrew ? ($user->name ?? '') : '') }}" class="ticket-input">
                        </div>
                        ...
                        <div>
                            ...
                            <input id="reporter_phone" type="text" name="reporter_phone" value="{{ old('reporter_phone') }}"
                                placeholder="Contoh: 08123456789" class="ticket-input">
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-2 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                                ...
                                <div class="grid gap-3 sm:grid-cols-3">
                                    ...
                                    <button type="button" onclick="openCameraBack()"
                                        class="photo-action bg-emerald-600 text-white hover:bg-emerald-700">
                                        <span class="text-base">📷</span>
                                        Kamera Belakang
                                    </button>

                                    <button type="button" onclick="openCameraFront()"
                                ...
                                <p class="mt-3 text-xs leading-5 text-slate-500">
                                    Bisa tambah foto berkali-kali dari gallery maupun kamera. Semua foto yang muncul di
                                    preview akan ikut tersimpan.
                                </p>
                                ...
                                <input id="realPhotosInput" type="file" name="photos[]" multiple hidden>
                                ...
                                <div id="photoPreview" class="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">
                                </div>
                            </div>
            ...
            <section class="border-t border-slate-100 bg-slate-50/60 p-6">
                <div class="grid gap-5 lg:grid-cols-2">
                    <div>
                        ...
                        <textarea id="description" name="description" rows="5" class="ticket-input resize-none"
...
@push('scripts')
    ...
    <script>
        $(function () {
            // Ambil status role dari PHP ke JS
            const isCrewMode = {{ $isCrew ? 'true' : 'false' }};
            const outletSelect = $('#outlet_id');

            // Select2 HANYA diinisialisasi jika bukan mode Crew
            if (!isCrewMode) {
            ...
            // Tetap menyimpan array fallback agar jika Backoffice memilih provinsi secara manual, kota tetap terfilter
            const citiesByProvince = {
                'Jawa Timur': ['Surabaya', 'Sidoarjo', 'Gresik', 'Malang', 'Kediri', 'Sumenep'],
                'Jawa Tengah': ['Semarang', 'Solo', 'Kudus', 'Pekalongan', 'Sragen'],
            ...
            // Trigger fallback jika provinsi dipilih secara manual
            $('#province').on('change', function () {
                populateCities($(this).val());
            });

            // Fungsi auto-fill yang membaca dari database via attribute HTML
            function setKotaFromSelectedOutlet() {
                let kota = '';
                let provinsi = '';

                // Cara ambil data-kota & provinsi beda antara Crew (hidden input) dan Backoffice (select)
                if (isCrewMode) {
                    kota = outletSelect.data('kota') || '';
                    provinsi = outletSelect.data('provinsi') || '';
                } else {
                    const selectedOpt = outletSelect.find(':selected');
                    kota = selectedOpt.data('kota') || '';
                    provinsi = selectedOpt.data('provinsi') || '';
                }

                // 1. Set Provinsi
                if (provinsi) {
                    if ($('#province option[value="' + provinsi + '"]').length === 0) {
                        $('#province').append($('<option>').attr('value', provinsi).text(provinsi));
                    }
                ...
                // 2. Set Kota
                if (provinsi) {
                    // Gunakan populateCities agar dropdown kota tetap rapi terfilter sesuai provinsinya
                    populateCities(provinsi, kota);
                } else if (kota) {
                    // Fallback murni jika provinsi kosong dari database tapi kotanya ada
                    const citySelect = $('#city');
                    citySelect.empty().append($('<option>').attr('value', '').text('Pilih Kabupaten / Kota'));
                    citySelect.append($('<option>').attr('value', kota).text(kota));
                    citySelect.val(kota);
                }
            }

            // Trigger otomatis saat dropdown outlet diganti (Backoffice)
            if (!isCrewMode) {
                outletSelect.on('change', setKotaFromSelectedOutlet);
            }

            // Trigger otomatis saat halaman pertama kali di-load (berguna untuk auto-fill Crew)
            if (outletSelect.val()) {
            ...
            // Eksekusi trigger validasi awal
            const initialProvince = $('#province').val();
            const initialCity = @json(old('city'));

            if (initialProvince && !outletSelect.val()) {
        ...
        // ==========================================
        // SCRIPT UPLOAD FOTO (TIDAK ADA PERUBAHAN)
        // ==========================================
        let selectedPhotos = [];
