...
<style>
    :root {
        --menu-primary: #f59e0b; /* Amber */
        --menu-primary-light: #fef3c7;
        --menu-dark: #1e293b;
        --menu-gray: #64748b;
        --menu-light: #f8fafc;
        --menu-green: #10b981;
    ...
    .menu-hero {
        background: linear-gradient(135deg, var(--menu-primary), #fbbf24);
        border-radius: 20px;
        padding: 40px;
        color: white;
        margin-bottom: 25px;
...
<div class="menu-page">
    <section class="menu-hero">
        <div class="menu-kicker"><i class="bi bi-rocket-takeoff"></i> Analitik GO</div>
        <h1 class="menu-title">Dashboard Outlet Go</h1>
        <p class="menu-subtitle">Pemantauan data penjualan harian untuk Outlet Go. Data ditarik *real-time* dari Google Spreadsheet pada bulan berjalan ({{ $bulanInfo }}).</p>
    </section>

    <!-- Filter Bar -->
    <section class="filter-section" style="margin-bottom: 20px;">
        <form method="GET" style="display: flex; flex-wrap: wrap; gap: 15px; background: rgba(255,255,255,0.8); backdrop-filter: blur(10px); padding: 15px 20px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.03); border: 1px solid rgba(245, 158, 11, 0.2);">
    ...
    <style>
        ...
        .trend-bar { flex: 1; min-width: 35px; max-width: 60px; background: linear-gradient(180deg, #f59e0b, #d97706); border-radius: 6px 6px 4px 4px; position: relative; transition: 0.3s; cursor: pointer; }
        .trend-bar:hover { background: linear-gradient(180deg, #fbbf24, #f59e0b); transform: translateY(-3px); box-shadow: 0 4px 10px rgba(245, 158, 11, 0.3); }
    ...
    <div class="menu-card" style="margin-bottom: 20px;">
        <div class="section-head">
            ...
            <span class="pill">Grafik Batang (All Outlet Go)</span>
        </div>
    ...
    <div class="menu-card">
        <div class="section-head">
            ...
            <span class="pill">Realtime Spreadsheet Data</span>
        </div>
                ...
                <tbody id="outletGoTableBody">
                    @forelse($outlets as $outlet)
                        <tr class="outlet-row" 
                            ...
                            data-promo="{{ strtolower($outlet['promo_go']) }}">
                            ...
                            <td>
                                @if($outlet['promo_go'])
                                    <span class="tag-tipe">{{ $outlet['promo_go'] }}</span>
                                @else
                                    -
                                @endif
                            </td>
                            <td class="text-dim">{{ $outlet['kategori'] ?: '-' }}</td>
                            ...
                            @foreach($dateColumns ?? [] as $dateStr)
                                @php
                                    ...
                                    // Update prevRaw if it's not empty, so we compare with the last non-empty day, 
                                    // or just compare with yesterday even if yesterday was empty (0).
                                    // Let's compare with the exact previous calendar day.
                                    $prevRaw = $raw;
                                @endphp
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ 6 + count($dateColumns ?? []) }}" style="text-align: center; padding: 40px; color: #94a3b8;">
                                Belum ada data Outlet Go yang ditarik.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
