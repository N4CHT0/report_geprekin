{{-- resources/views/investor/laporan_perbulan.blade.php --}}
@section('title', 'Laporan Penjualan Bulanan')
@section('breadcrumb', 'Laporan / Penjualan Bulanan')
...
@php
    ...
    if (!empty($bulanTahun) && strpos($bulanTahun, '-') !== false) {
        [$tahun, $bulan] = explode('-', $bulanTahun);
        $jumlahHari = Carbon::create((int) $tahun, (int) $bulan, 1)->daysInMonth;
    }
            ...
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                ...
                <div class="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-end">
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                        ...
                        <div class="mt-0.5 text-sm font-extrabold text-slate-900">{{ $bulanTahun ?: 'Belum dipilih' }}</div>
                    </div>
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                        <div class="text-[11px] font-bold uppercase tracking-wide text-slate-500">Jumlah Hari</div>
                        <div class="mt-0.5 text-sm font-extrabold text-slate-900">{{ $jumlahHari }} Hari</div>
                    </div>
                    <div class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
        ...
        <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div class="flex items-center justify-between border-b border-slate-200 p-4">
                <div>
                    ...
                    <p class="mt-1 text-xs font-medium text-slate-500">Atur periode, outlet, dan ecommerce.</p>
                </div>
            ...
            <form
                ...
                x-transition
                method="GET"
                action="{{ route('investor.laporan.perbulan') }}"
                id="formLaporanBulan"
                class="p-4"
            >
                <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
                    <div class="lg:col-span-3">
                        <label for="bulan_tahun" class="mb-2 block text-xs font-extrabold uppercase tracking-wide text-slate-600">
                            Bulan & Tahun
                        </label>
                        <input
                            type="month"
                    ...
                    <div class="lg:col-span-5">
                        ...
                        <div class="grid grid-cols-2 gap-2 sm:grid-cols-4">
                            ...
                            <a
                                href="{{ route('investor.laporan.perbulan') }}"
                            ...
                            <a
                                href="{{ route('investor.laporan.perbulan.export', [
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-3">
                            <div class="mb-3">
                                ...
                                <div class="text-xs font-medium text-slate-500">
                                    Kosongkan checklist ecommerce jika ingin menampilkan sales normal dari laporan bulanan.
                                </div>
                            </div>
                                ...
                                <thead class="sticky top-0 z-20">
                                    <tr class="bg-slate-100 text-center text-[11px] font-extrabold uppercase tracking-wide text-slate-600">
                                        ...
                                        @for ($d = 1; $d <= $jumlahHari; $d++)
                                            <th colspan="4" class="border-b border-r border-slate-200 bg-slate-200/70 px-3 py-3">{{ sprintf('%02d', $d) }}</th>
                                        @endfor
                                ...
                                <tbody class="divide-y divide-slate-100 bg-white">
                                    @forelse ($tableData as $index => $outlet)
                                        ...
                                        >
                                            ...
                                            @for ($d = 1; $d <= $jumlahHari; $d++)
                                                @php
                                                    $sales = $outlet['hari'][$d]['sales'] ?? 0;
                                                    $cu = $outlet['hari'][$d]['cu'] ?? 0;
                                                    $avg = $outlet['hari'][$d]['ac'] ?? 0;
                                                    $avgSize = $outlet['hari'][$d]['basket_size'] ?? 0;
                                                @endphp
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="{{ 6 + ($jumlahHari * 4) + 4 }}" class="px-4 py-10 text-center">
                                                <div class="text-base font-extrabold text-slate-800">Data tidak tersedia</div>
                                                <div class="mt-1 text-sm font-medium text-slate-500">Silakan pilih periode atau ubah filter.</div>
                                            </td>
                                        </tr>
                                    @endforelse
                                    ...
                                    <tr class="bg-blue-600 text-white">
                                        ...
                                        @for ($d = 1; $d <= $jumlahHari; $d++)
                                            @php
                                                $gsales = $grandTotal['hari'][$d]['sales'] ?? 0;
                                                $gcu = $grandTotal['hari'][$d]['cu'] ?? 0;
                                                $gac = $grandTotal['hari'][$d]['ac'] ?? 0;
                                                $gbasket = $grandTotal['hari'][$d]['basket_size'] ?? 0;
                                            @endphp
